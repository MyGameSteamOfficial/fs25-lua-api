...
--              TODO: missing setRenderedInViewports in binding
    ...
    if self.isServer and self.savegame ~= nil then
        if self.savegame.xmlFile:getString(self.savegame.key .. "#mapBoundId") ~= nil then
...
function Placeable:getSellAction()
    local isOnPublicGround = g_farmlandManager:getFarmlandOwner(self:getFarmlandId()) == FarmManager.SPECTATOR_FARM_ID

    -- keep placeables on farm 0 if they are on public ground (e.g. prod points) or cannot be deleted and
    if self:getIsPreplaced() and (isOnPublicGround or not self.canBeDeleted) then
