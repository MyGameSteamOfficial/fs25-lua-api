...
function PlaceablePlacement:getHasOverlap(x, y, z, rotY, checkFunc)
    ...
    for _, area in ipairs(spec.testAreas) do
        ...
        if g_densityMapHeightManager:getIsPlacementAreaBlocked(frontLeftX, frontLeftZ, frontRightX, frontRightZ, backLeftX, backLeftZ) then
            return true, nil
        end
        ...
        overlapBox(posX, posY, posZ, 0, rotY + area.rotYOffset, 0, extendX, extendY, extendZ, "overlapCallback", callbackTarget, PlaceablePlacement.OVERLAP_COLLISION_MASK, true, true, true, true)
