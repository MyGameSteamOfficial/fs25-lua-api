...
function PlaceableRiceField:loadFromXMLFile(xmlFile, key)
    for fieldIndex, fieldKey in xmlFile:iterator(key .. ".fields.field") do
        ...
        if self.isServer and not g_currentMission.missionInfo.isValid then
            ...
            Logging.devInfo("perform initial deformation for rice field %q with %q at %d", key, initialFruitName, field.initialFruitGrowthState)
...
-- @return list? nodesToVisualizeByBrush
...
function PlaceableRiceField:buildMesh(field)
    ...
    setCollisionFilter(placementColPlaneNode, CollisionFlag.PLACEMENT_BLOCKING + CollisionFlag.GROUND_TIP_BLOCKING, 1)
