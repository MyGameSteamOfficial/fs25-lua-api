...
function WheelDebug:fillDebugValues(debugTable)
    ...
    local tireLoad = getWheelShapeContactForce(self.wheel.node, physics.wheelShape)
    if tireLoad ~= nil then
        local nx, ny, nz = getWheelShapeContactNormal(self.wheel.node, physics.wheelShape)
        local dx, dy, dz = localDirectionToWorld(self.wheel.node, physics.directionX, physics.directionY, physics.directionZ)
        tireLoad = -tireLoad*MathUtil.dotProduct(dx, dy, dz, nx, ny, nz)

        tireLoad = tireLoad + math.max(ny*gravity, 0.0) * self.wheel:getMass() -- add gravity force of tire
    else
        tireLoad = 0
    end
