...
function WheelPhysics:readStream(streamId, updateInterpolation)
    ...
    self.hasSoilContact = streamReadBool(streamId)
end
...
function WheelPhysics:writeStream(streamId)
    ...
    streamWriteBool(streamId, self.hasSoilContact)
end
...
function WheelPhysics:updateContact()
    ...
    if g_updateLoopIndex - self.wheelShapeCreationFrameIndex > 2 then
        ...
        if contactObject ~= 0 then
            ...
            elseif self.hasGroundContact then
                ...
                self.lastContactObjectAllowsTireTracks = getRigidBodyType(contactObject) == RigidBodyType.STATIC and getUserAttribute(contactObject, "noTireTracks") ~= true
            else
                self.contact = WheelContactType.NONE
    ...
    if self.contact == WheelContactType.GROUND then
        ...
        local densityType = bitAND(bitShiftRight(self.densityBits, groundTypeFirstChannel), 2^groundTypeNumChannels - 1)
...
function WheelPhysics:updateBase()
    if self.vehicle.isServer and self.vehicle.isAddedToPhysics then
        ...
        self.wheelShape = createWheelShape(self.wheel.node, positionX, positionY, positionZ, self.radius, self.suspTravel, self.spring, self.damperCompressionLowSpeed, self.damperCompressionHighSpeed, self.damperCompressionLowSpeedThreshold, self.damperRelaxationLowSpeed, self.damperRelaxationHighSpeed, self.damperRelaxationLowSpeedThreshold, self.wheel:getMass(), collisionGroup, collisionMask, self.wheelShape)
...
---
function WheelPhysics:updateTick(dt, groundWetness)
...
---
function WheelPhysics:clientUpdate(dt, interpolationAlpha)
