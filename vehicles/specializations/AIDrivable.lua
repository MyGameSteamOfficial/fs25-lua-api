...
function AIDrivable:onPostLoad()
    ...
    if self.xmlFile:hasProperty("vehicle.ai.agent") then
...
function AIDrivable:loadAgentInfoFromXML(xmlFile, agent)
    ...
    for name, id in pairs(self.configurations) do
        ...
        agent.frontWheelIndices = xmlFile:getValue(baseSizeKey .. "#frontWheelIndices", agent.frontWheelIndices, true)
        agent.frontWheelNodes = xmlFile:getValue(baseSizeKey .. "#frontWheelNodes", agent.frontWheelNodes, self.components, self.i3dMappings, true)
