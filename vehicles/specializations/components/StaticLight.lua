...
function StaticLight:setLightTypesMask(lightsTypesMask)
    ...
    if self.vehicle:getIsLightActive(self) then
        ...
        for _, lightType in pairs(self.lightTypes) do
            if bitAND(lightsTypesMask, 2 ^ lightType) ~= 0 or (lightType == -1 and self.vehicle:getIsActiveForLights(true)) then
    ...
    if not self.useLightControlShaderParameter then
        if self.absUVSlotIndex == nil then
            local lightTypeBitMask = 0
             ...
             -- set all to blinking
            for i=0, 11 do
                lightTypeBitMask = bitOR(lightTypeBitMask, bitShiftLeft(StaticLightCompoundLightType.BLINKING - 1, i * 2))
            end
            ...
            if self.absUVSlotIndex <= 12 then
                ...
                local mask = (2 ^ (12 * 2) - 1) - bitShiftLeft(0x3, bitIndex)
                lightTypeBitMask = bitAND(lightTypeBitMask, mask) -- erase
                lightTypeBitMask = bitOR(lightTypeBitMask, bitShiftLeft(StaticLightCompoundLightType.BLINKING - 1, bitIndex)) -- set to blinking
                setShaderParameter(self.node, "lightTypeBitMask", lightTypeBitMask, nil, nil, nil, false)
            end
        end
    end
