...
--#profile     RemoteProfiler.zoneBeginN("Crawlers-updateCrawler-updateAlignment")
    if crawler.referenceNode ~= nil then
        if self.currentUpdateDistance < Crawlers.MAX_UPDATE_DISTANCE_ALIGNMENT then
            setWorldTranslation(crawler.linkNode, getWorldTranslation(crawler.positionReferenceNode))
            local x, y, z = getWorldTranslation(crawler.loadedCrawler)
            ...
            local upX, upY, upZ = localDirectionToWorld(crawler.linkNode, 0, 1, 0)
            setWorldDirection(crawler.loadedCrawler, dx, dy, dz, upX, upY, upZ)
        end
    end
--#profile     RemoteProfiler.zoneEnd()
end
