...
function Motorized.initSpecialization()
    ...
    local valueTypes = {
        "rpm", "load", "speed", "speedDir", "fuelUsage", "motorTemperature", "motorTemperatureWarning", "clutchPedal", "gear", "gearGroup", "gearShiftUp", "gearShiftDown",
...
function Motorized:getName(superFunc)
    ...
    local configItem = ConfigurationUtil.getConfigItemByConfigId(self.configFileName, "motor", self.configurations["motor"])
    if configItem ~= nil then
        if not configItem.hasDefaultName then
