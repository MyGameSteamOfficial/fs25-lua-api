...
function ConnectionHoses:loadHoseTargetNode(xmlFile, targetKey, entry)
    ...
    if socketName ~= nil then
        local socketMaterial = xmlFile:getValue(targetKey .. "#socketMaterialTemplateName")
...
function ConnectionHoses:loadHoseNode(xmlFile, hoseKey, entry, isBaseHose)
    ...
    entry.material = xmlFile:getValue(hoseKey.."#materialTemplateName")
    entry.adapterMaterial = xmlFile:getValue(hoseKey.."#adapterMaterialTemplateName")
    ...
    if socketName ~= nil then
        local socketMaterial = xmlFile:getValue(hoseKey .. "#socketMaterialTemplateName")
        entry.socket = g_connectionHoseManager:linkSocketToNode(socketName, entry.node, self.customEnvironment, socketMaterial)
