        ...
        if savegame ~= nil and savegame.xmlFile:hasProperty(savegame.key..".fillUnit") then
            ...
            while true do
                ...
                local allowLoading = fillUnitsToLoad[fillUnitIndex] == nil or -- always load (fill unit with start fill level)
                                    (fillUnitsToLoad[fillUnitIndex] ~= nil and not savegame.resetVehicles) -- only load if not reset (fill unit without start fill level)
...
function FillUnit:unloadFillUnits(ignoreWarning)
    if not self.isServer then
        ...
        if spec.unloadingFillUnitsRunning then
            ...
            g_server:broadcastEvent(FillUnitUnloadedEvent.new(self, nil, false, false), false, nil, self)
            return
        end
        ...
        for fillUnitIndex, fillUnit in ipairs(self:getFillUnits()) do
            ...
            if fillUnit.canBeUnloaded and self:getFillUnitFillLevel(fillUnitIndex) > 0 and (targetFillUnitIndex == nil or targetFillUnitIndex == fillUnitIndex) then
...
function FillUnit:getFillLevelInformation(superFunc, display)
    ...
    for i=1, #spec.fillUnits do
        ...
        if fillUnit.capacity > 0 and fillUnit.showOnHud then
            ...
            display:addFillLevel(fillType, fillLevel, capacity, fillLevel > 0 and fillUnit.uiPrecision or 0, maxReached, fillUnit.uiDisplayTypeId, fillUnit.uiCustomFillTypeName)
        end
    end
end
