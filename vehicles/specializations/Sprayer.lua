...
function Sprayer:getExternalFill(fillType, dt)
    ...
    local allowHerbicide = self:getFillUnitAllowsFillType(self:getSprayerFillUnitIndex(), FillType.HERBICIDE)
...
function Sprayer:onStartWorkAreaProcessing(dt)
    ...
    if isExternallyFilled then
        -- only consume while ai is on field, not while turning or being blocked
        if self.rootVehicle.getAIFieldWorkerIsTurning == nil
        or (not self.rootVehicle:getAIFieldWorkerIsTurning() and not self.rootVehicle:getAIFieldWorkerIsBlocked()) then
