...
function Washable.initSpecialization()
    ...
    schema:register(XMLValueType.FLOAT, "vehicle.washable#wetDuration", "Duration until fully wet (minutes)", 2)
    schema:register(XMLValueType.FLOAT, "vehicle.washable#dryDuration", "Duration until the vehicle is fully dry again (minutes)", 10)
...
function Washable:onLoad(savegame)
    ...
    spec.wetDuration = self.xmlFile:getValue("vehicle.washable#wetDuration", 2) / 60 / 1000
    spec.dryDuration = 1 / math.max(self.xmlFile:getValue("vehicle.washable#dryDuration", 10) * 60 * 1000, 0.0001)
    ...
    if self.propertyState == VehiclePropertyState.SHOP_CONFIG then
        SpecializationUtil.removeEventListener(self, "onLoadFinished", Washable)
